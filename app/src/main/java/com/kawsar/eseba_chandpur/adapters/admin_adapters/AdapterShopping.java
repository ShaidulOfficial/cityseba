...
import com.kawsar.eseba_chandpur.views.fragments.edit_fragments.ShoppingEditFragment;

import java.util.List;

public class AdapterShopping extends RecyclerView.Adapter<AdapterShopping.ViewHolderShop> {
    Context context;
    List<CommonModel> commonModelList;


    public AdapterShopping(Context context, List<CommonModel> modelList) {
        this.context = context;
        this.commonModelList = modelList;
    }

    public void setFilterList_shopping(List<CommonModel> filterList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolderShop holder, int position) {
        ...
        Glide.with(context).load(imageUrl).placeholder(R.drawable.chandpur)
                .into(holder.shoppingPaperLogo);
        Animation animation = AnimationUtils.loadAnimation(holder.itemView.getContext(), R.anim.anim1);
        holder.itemView.setAnimation(animation);

        holder.shoppingPaperLogo.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        holder.editBtnShopping.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Bundle bundleNews = new Bundle();
                bundleNews.putString("uIdShop", userId);
                bundleNews.putString("dataTypeShop", dataType);
                AppCompatActivity activity = (AppCompatActivity) v.getContext();
                Fragment shoppingEditFragment = new ShoppingEditFragment();
                shoppingEditFragment.setArguments(bundleNews);
                activity.getSupportFragmentManager().beginTransaction()
                        .replace(R.id.framlayout_admin_dashboard, shoppingEditFragment)
                        .addToBackStack(null)
                        .commit();
            }
        });
        holder.deleteBtnShopping.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
    ...
    public class ViewHolderShop extends RecyclerView.ViewHolder {
        ...
        ImageView shoppingPaperLogo, deleteBtnShopping, editBtnShopping;

        public ViewHolderShop(@NonNull View itemView) {
            super(itemView);
            shoppingPaperLogo = itemView.findViewById(R.id.newsImageLogoItem);
            deleteBtnShopping = itemView.findViewById(R.id.delete_item_news);
            editBtnShopping = itemView.findViewById(R.id.edit_item_news);
